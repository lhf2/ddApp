$(document).ready(function(){
    ...
    if(location.href.indexOf('zjx.html')>=0){
        ...
        $('.my-location').click(function(){
            ...
            $('#city-detail').attr('lang',localStorage.getItem('city'));
        });
        ...
        $('.keyword-text').click(function () {
            if(citycur==""){
                    citycur=$('#city-detail').attr('lang');
            }else{
                    citycur=localStorage.getItem('city');
            }
       ...
       $('.search-btn').click(function () {
           ...
           if(sessionStorage.getItem('pricehtml')){
               var sesspricehtml = sessionStorage.getItem('pricehtml');
               if(sessionStorage.getItem('pricehtml')[0] == "￥"){
                pricef = sesspricehtml.replace("￥","");
               }
           }
           ...
           if(mylocal==localStorage.getItem('mylocation')){
            ...
            if(citycur==undefined){
                 citycur=localStorage.getItem('city');
            }
            //debugger
            console.log("citycur:"+citycur);
    ...
    if(location.href.indexOf('cxym.html')>=0){
        ...
        if(citycur1==""){
             citycur1=getQueryString('city');
        }else{
             citycur1=localStorage.getItem('area');
        }
         if(citycur==""){
             citycur=getQueryString('city');
        }else{
             citycur=localStorage.getItem('city');
        }
    }
   ...
   // area(citycur1);
    ...
    var district='';
    var wom=getQueryString('wom');
    var licenseLevel='';
    var distance='';
    var peopleNum='';
    var keyw=getQueryString('keyw');
    var mylocation=getQueryString('myl');
    //searchconditions();
    //根据筛选条件来查询
    ...
    function searchconditions(){
       ...
       // debugger
        ...
        if(location.href.indexOf('cxym.html')>=0){
            ...
            $(".main-detail-box").scroll(function(){
                var juli = ($(this).scrollTop()+$(this).height());
                var height = $(".main-detail").height();
                var page = parseInt($('.main-detail').attr("ispage"));
                if(cxymisDown && juli>=height-10 || juli>=height+10){
                    page++;
                    $('.main-detail').attr("ispage",page);
                    zjxAJAX({});
                }
           });
        }
            ...
            function zjxAJAX(_upOrDown){
                ...
                keyw=keyw==undefined?'':keyw;
                areacur=areacur==undefined?'':areacur;
                sortType=sortType==undefined?'':sortType;
                wom=wom==undefined?'':wom;
                licenseLevel=licenseLevel==undefined?'':licenseLevel;
                haveBus=haveBus==undefined?'':haveBus;
                haveExamSite=haveExamSite==undefined?'':haveExamSite;
                ...
                latitude=latitude==undefined?'0':latitude;
                 longitude=longitude==undefined?'0':longitude;
                  peopleNum=peopleNum==undefined?'0':peopleNum;
                   ispage=ispage==undefined?'0':ispage;
                /* console.log("citycur:"+citycur);
                  console.log("keyw:"+keyw);
                    console.log("areacur:"+areacur);
                ...
                $.ajax({
                    ...
                    url : 'http://dd.dingdongxueche.com/DDWeb/drivingInfo/selectByConditions.htm?city='+citycur
                            +'&keyword='+keyw
                            +'&price='+price
                            +'&area='+areacur
                            +'&district='+mylocation
