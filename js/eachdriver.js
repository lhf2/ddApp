    ...
    function eachdriving(did){
        $.ajax({
            ...
            success : function(msg) {
                ...
                //驾校详细信息
				var trainAddress = msg.trainAddress;
                if(trainAddress[0] == "[" && trainAddress.indexOf("[") != -1 &&  trainAddress.indexOf("]") != -1){
						trainAddress = trainAddress.replace(new RegExp(/(&quot;)/g),'"');
						trainAddress = trainAddress.replace(new RegExp(/(&QUOT;)/g),'"');
						trainAddress = trainAddress.replace(new RegExp(/(｛)/g),'{');
						trainAddress = trainAddress.replace(new RegExp(/(﹜)/g),'}');
						trainAddress = eval(trainAddress);
				}
				//写到这里
                $('.list-all').eq(0).html(msg.trainAddress);
                var trainMes = msg.trainAddress;
                var trainStr = '';
                if(typeof trainMes == "object") {
                    var len = trainMes.length;                        
                    for(var i = 0; i < len; i++){
                        trainStr+='<p>'+(trainMes[i].trainAddress)+'</p>';
                    }
                }else{
                    trainStr+='<p>'+(trainMes)+'</p>';
                }
        ...
        $.ajax({
            ...
            success : function(msg) {
                ...
                for(var i=0;i<msg.length;i++){
                    ...
                    $('.summary-con').eq(i).html(msg[i].drivingtype);
                    // var classname = msg[i].classname.split("班");
                    // if(msg[i].classname.indexOf("班") != -1){
                    //     $('.summary-top').eq(i).html(classname[0]+"班");
                    //     $('.summary-con').eq(i).html(classname[1]);
                    // }else{
                    //     $('.summary-top').eq(i).html(msg[i].classname);
                    //     $('.summary-con').eq(i).remove();
                    // }
