$(function(){
    ...
    if(location.href.indexOf('index00.html')>=0){
        var vtype1=getQueryString('vtype') || "小车";
    ...
    $('.bottom-btn').click(function () {
        ...
        window.location.href = 'http://wx.dingdongxueche.com/index00.html?vtype='+vtype;
    });
    //重制图片
    function reloadimg(){
    ...
    $(".option-list").click(function(){
        ...
        switch(txt){
            case "小车": 
            str = "科目一共1311题，科目四共1121题"
            break;
            case "货车": 
            str = "科目一共1382题，科目四共1124题"
            break;
            case "客车": 
            str = "科目一共1374题，科目四共1124题"
            break;
            case "摩托车": 
            str = "科目一共400题，科目四共204题"
            break;

        }
        $(".main-bot span").eq(0).text(str);
    });
    //导航栏
    var stype;
    var pages = $(".page").attr("page");//当前页数
    $("#fix-both .kmclick").click(function(){
        var index = $(this).index()+1;
        $(".page").attr("page",index);
        $('#fix-both ul li a').removeClass('cur');
        $(this).children('a').addClass('cur');
        stype=$('#fix-both ul li a.cur').html();
        var box = $(".thisis-main-box-parentbox");
        var child = box.find(".isparents");
        child.hide();
        switch (index){
            case 1:
                child.eq(0).show();
            ...
            case 2:
                child.eq(1).show();
                $("#single-text").text("科目二视频");
                $("#single-Rules-text").text("科二考规")
                break;
            case 3:
                child.eq(1).show();
                $("#single-text").text("科目三视频");
                $("#single-Rules-text").text("科三考规")
                break;
            case 4:
                child.eq(0).show();
                $("#isnum").text("科四考规");
    ...
    // })
    //科目
    var vehicleType=getQueryString('vtype') || "小车";
    var urlCity=localStorage.getItem("city") || "北京";
    //模拟考试
    $('.big-circle').click(function () {
         window.location.href = "index-topic/topic.html?&stype="+stype+"&testtype=4&city="+urlCity;
    ...
    $('.ling1').click(function(){
        ...
        window.location.href = "index-topic/topic.html?&stype="+stype+"&testtype=0&city="+urlCity;
    })
    //随机练习
    $('.ling2').click(function(){
        //$(this).children('a').attr('href','kmy/sjlx.html?testtype=sjlx&stype='+stype+'&vtype='+vehicleType+'&swiperindex=0')
        window.location.href = "index-topic/topic.html?&stype="+stype+"&testtype=1&city="+urlCity;
    })
    //章节练习
