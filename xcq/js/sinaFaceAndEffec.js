	...
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			if($('#emotions .categorys')[0]){
				$('#emotions').css({width:'99%',height:'50%',overflow:'auto',zIndex:'1000'});
				$('#emotions').css({top: eTop, left: 0});
				$('#emotions').toggle();
				return;
			}
			$('body').append('<div id="emotions"></div>');
			$('#emotions').css({width:'99%',height:'50%',overflow:'auto'});
			$('#emotions').css({top: eTop, left: 0});
		...
		$('body').click(function(){
			$('#emotions').remove();
			$('.comment-bottom').css('bottom',0+'px');
			$('.btn-bottom').css('bottom',0+'px');
		});
